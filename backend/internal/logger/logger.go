...
func init() {
	...
	if err != nil {
		log.Fatal("no se pudo cargar la configuracion: ", err)
	}
	AppPathLog, err := c.Get("app_path_log")
	if err != nil {
		log.Fatal("no se pudo cargar configuracion app_path_log: ", err)
		return
	}
	AppServiceName, err := c.Get("app_service_name")
	if err != nil {
		log.Fatal("no se pudo cargar configuracion app_service_name: ", err)
		return
	}
	logReviewInterval, err = c.GetInt("app_log_review_interval")
	if err != nil {
		fmt.Println("no se pudo cargar configuracion app_service_name: ", err)
		return
	}
